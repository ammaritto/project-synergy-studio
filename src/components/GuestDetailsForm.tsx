...
// Custom hook to manage dropdown state and prevent height issues
const useDropdownHeightFix = () => {
  const [isOpen, setIsOpen] = useState(false);

  useEffect(() => {
    if (isOpen) {
      // When dropdown opens, inform parent iframe to ignore height changes temporarily
      const isInIframe = window.parent !== window;
      if (isInIframe) {
        window.parent.postMessage({
          type: 'dropdown-state',
          isOpen: true,
          action: 'ignore-height-changes'
        }, '*');
      }
    } else {
      // When dropdown closes, allow height changes again after a delay
      const isInIframe = window.parent !== window;
      if (isInIframe) {
        setTimeout(() => {
          window.parent.postMessage({
...
const GuestDetailsForm: React.FC<GuestDetailsFormProps> = ({
  ...
  // Use dropdown height fix hook
  const { isOpen, setIsOpen } = useDropdownHeightFix();
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="pt-[10px] pb-[10px] col-span-full">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Enhanced Phone Field with Dropdown Height Fix */}
            <div>
                ...
                <div className="relative w-20 sm:w-28 md:w-32">
                  ...
                  >
                    ...
                    <SelectContent 
                      ...
                      sideOffset={5}
                      onCloseAutoFocus={(e) => {
                        // Prevent focus issues that might trigger height recalculation
                        e.preventDefault();
                      }}
                    >
