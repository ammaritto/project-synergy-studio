...
const GuestDetailsForm: React.FC<GuestDetailsFormProps> = ({
  ...
  useEffect(() => {
    if (isInIframe()) {
      const cleanup = setupParentMessageListener((message) => {
        switch(message.type) {
          case 'GUEST_DETAILS_SUBMITTED':
            if (message.data) {
              setGuestDetails(message.data);
  ...
  }, [setGuestDetails, onSubmit, onBack]);
  ...
  const modalData = {
    unitName: selectedUnit.inventoryTypeName,
    buildingName: selectedUnit.buildingName,
    checkIn: formatDateWithWeekday(confirmedSearchParams.startDate),
    checkOut: formatDateWithWeekday(confirmedSearchParams.endDate),
    nights: calculateNights(),
    totalPrice: formatCurrency(selectedUnit.selectedRate.totalPrice)
  };
